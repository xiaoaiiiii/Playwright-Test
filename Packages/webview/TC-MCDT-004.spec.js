...
const stepName1 = async ({ page, testInfo, ...options }) => {
  await test.step("导入文件", async (stepInfo) => {
    ...
    await page.evaluate(() => { pywebview.api.choosePath = () => Promise.resolve(JSON.stringify({ status: "ok", directory: "D:/test-data/sample/testfile_002.blf" })) }); await page.locator('div#app > div.o-body:nth-child(2) > div.blf-check:nth-child(1) > div.record-header:nth-child(1) > div.record-header-right:nth-child(2) > div.o-btn:nth-child(1)').click();
...
test.handleTestOperate({
  ...
  requireAnnotations,
  annotations: {
    inputConditions: "不同DBC解析通道（MS11-U / 25RC10）",
