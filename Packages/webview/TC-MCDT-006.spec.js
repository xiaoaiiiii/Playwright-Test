...
const requireAnnotations = {
  ...
  step: `
          导入不同车型 BLF 文件使用不同dbc模板进行解析（正确的dbc模板解析）。
        `,
};
const stepName1 = async ({ page, testInfo, ...options }) => {
  await test.step("导入文件", async (stepInfo) => {
    ...
    await page.locator('div#app > div.o-body:nth-child(2) > div.home-page:nth-child(1) > div.home-tools:nth-child(2) > a.home-tools-box:nth-child(1)').click()

    await page.handle.waitTime(500);

    await page.evaluate(() => { pywebview.api.choosePath = () => Promise.resolve(JSON.stringify({ status: "ok", directory: "D:/test-data/sample/testfile_003.blf" })) };})) }); await page.locator('div#app > div.o-body:nth-child(2) > div.blf-check:nth-child(1) > div.record-header:nth-child(1) > div.record-header-right:nth-child(2) > div.o-btn:nth-child(1)').click();
...
const stepName2 = async ({ page, testInfo, ...options }) => {
  await test.step("选择dbc模板", async (stepInfo) => {
    ...
    try {
      ...
      expect(await page.locator('div#app > div.o-body:nth-child(2) > div.blf-check:nth-child(1) > div.o-import-dialog:nth-child(3) > div.o-dialog-wrap:nth-child(1) > div.o-dialog-container-wrap:nth-child(2) > div.o-import-configuration:nth-child(1) > div.o-dialog-item-wrap:nth-child(3) > div.o-dialog-item-select:nth-child(2) > div.v-select:nth-child(1) > div.vs__dropdown-toggle:nth-child(1) > div.vs__selected-options:nth-child(1) > span.vs__selected:nth-child(1)')).toHaveText('MX11 / 25RC10')
    ...
    await page.handle.addStepAnnotations({
      operate: "选择dbc模板",
      expected: "选择模板为MX11 / 25RC10, 并校验dbc模板是否正确",
      status: stepStatus,
    });
  });
};
...
const stepname3 = async ({ page, testInfo, ...options }) => {
  await test.step("选择快速适应模式", async (stepInfo) => {
    let stepStatus = true;

    // 阶段操作

    await page.locator('div#app > div.o-body:nth-child(2) > div.blf-check:nth-child(1) > div.o-import-dialog:nth-child(3) > div.o-dialog-wrap:nth-child(1) > div.o-dialog-container-wrap:nth-child(2) > div.o-import-configuration:nth-child(1) > div.o-dialog-item-wrap:nth-child(3) > div:nth-child(3) > label.o-input:nth-child(3) > input:nth-child(1)').click()

    try {
      const isChecked = await page.locator('div#app > div.o-body:nth-child(2) > div.blf-check:nth-child(1) > div.o-import-dialog:nth-child(3) > div.o-dialog-wrap:nth-child(1) > div.o-dialog-container-wrap:nth-child(2) > div.o-import-configuration:nth-child(1) > div.o-dialog-item-wrap:nth-child(3) > div:nth-child(3) > label.o-input:nth-child(3) > input:nth-child(1)').isChecked();
      if (isChecked) {
        console.log('验证通过：快速自适应单选按钮已被选中');
        stepStatus = true;
      } else {
        console.log('验证失败：快速自适应单选按钮未被选中');
        stepStatus = false;
      }
    } catch (error) {
      stepStatus = false;
      console.error('验证快速自适应单选按钮状态时出错:', error.message);
    }

    await page.handle.addStepAnnotations({
      operate: "点击快速适应模式",
      expected: "进入快速适应模式并成功解析数据",
      status: stepStatus,
    });
  });
};
    ...
    try {
      if (!page || page.isClosed()) {
        ...
        await page.handle.addStepAnnotations({
          ...
          expected: "CAN编号和Channel编号的数字至少有一行数据不一致",
          status: stepStatus,
          error: "页面已关闭，无法进行校验"
        });
        return;
      ...
      // 校验每行对应位置的数字是否一致 - 有一行不匹配即通过，跳过第一行
      ...
      if (validRowCount === 0) {
        ...
        console.log(`\n验证通过：在 ${validRowCount} 个有效数据行中，发现 ${mismatchIndices.length} 个位置的数字不一致`);
        stepStatus = true;
      } else {
        console.log(`\n验证失败：在 ${validRowCount} 个有效数据行中，没有发现不匹配的数字`);
        stepStatus = false;
      }
...
test.handleTestOperate({
  ...
  requireAnnotations,
  annotations: {
    inputConditions: "不同DBC解析通道（MX11 / 25RC10）",
    expectedResults: "测试失败",
  },
  params: {
    testStatus: false,
    testValue: 0,
