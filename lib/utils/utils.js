...
/**
 * 获取当前系统可用的 Python 命令名
 * - macOS / Linux 通常只有 python3
 * - Windows 通常是 python
 * 优先使用 python3，回退到 python，都没有则返回 "python"（让上游报错时仍能保留原行为）
 * @returns {string}
 */
...
export function checkPort(port) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      if (output && output.trim()) resolve(output);
      else resolve(false);
    } catch (error) {
      // 命令非 0 退出在 lsof / findstr 都代表"没有匹配"，等价于端口空闲
      resolve(false);
    }
  });
}
...
/**
 * 释放端口：找到占用指定端口的进程并 kill。
 * 跨平台（macOS / Linux 用 lsof + kill；Windows 用 netstat + taskkill）。
 * @param {number} port
 * @returns {Promise<number[]>} 被 kill 的 PID 列表
 */
